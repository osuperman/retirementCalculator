...
import { ReviewContext } from './ui/ReviewContext.js';
import { fieldReviews } from './ui/reviewModel.js';
...
export default function FinancialDetails({values,onChange,household=false,personOnly=false,scope=''}) {
  ...
  const fieldId = useId();
  const owner = household ? 'shared' : personOnly ? scope.toLowerCase() : 'individual';
  const pending = key => fieldReviews(reviews, owner, key);
  const attributes = (key, unconfirmed = false) => ({
    id: `${fieldId}-${key}`, 'data-needs-review': pending(key).length > 0 || undefined,
    'aria-describedby': pending(key).length || unconfirmed ? `${fieldId}-${key}-note` : undefined,
  });
  const marker = (key, unconfirmed = false) => pending(key).length
    ? <small id={`${fieldId}-${key}-note`} className="field-review-note">Needs review: {pending(key).map(item => item.impact).join('; ')}</small>
    : unconfirmed ? <small id={`${fieldId}-${key}-note`} className="field-unconfirmed-note">Unconfirmed</small> : null;
  const startYear=values.projectionStartYear ?? new Date().getFullYear();
  const number=(key,label,nullable=false)=> <label data-review-field={key} className="block text-xs text-slate-700" key={key}>{label}<NumericField {...attributes(key, nullable && values[key] == null)} className={inputClass} min="0" step="any" nullable={nullable} data-unconfirmed={nullable && values[key] == null} placeholder={nullable ? 'Unconfirmed' : undefined} value={values[key]} onValue={onChange(key)}/>{marker(key, nullable && values[key] == null)}</label>;
  const select=(key,label,options)=><label data-review-field={key} className="block text-xs text-slate-700" key={key}>{label}<select {...attributes(key, (values[key] ?? options[0][0]) === 'unknown')} className={inputClass} data-unconfirmed={(values[key] ?? options[0][0]) === "unknown"} value={values[key] ?? options[0][0]} onChange={e=>onChange(key)(e.target.value)}>{options.map(([value,text])=><option key={value} value={value}>{text}</option>)}</select>{marker(key, (values[key] ?? options[0][0]) === 'unknown')}</label>;
  const check=(key,label)=><label data-review-field={key} className="flex gap-2 text-xs text-slate-700" key={key}><input {...attributes(key)} type="checkbox" checked={values[key]===true} onChange={e=>onChange(key)(e.target.checked)}/><span>{label}{marker(key)}</span></label>;
  const date=(key,label)=><label data-review-field={key} className="block text-xs text-slate-700" key={key}>{label}<input {...attributes(key)} className={inputClass} type="date" value={values[key] || ''} onChange={e=>onChange(key)(e.target.value)}/>{marker(key)}</label>;
  const history=(key,label,years)=><fieldset data-review-field={key} className="space-y-2"><legend className="font-medium text-sm">{label}</legend>{marker(key)}{years.map(year=><label className="block text-xs" key={year}>{year}<input data-needs-review={pending(key).length > 0 && values[key]?.[year] == null || undefined} aria-describedby={pending(key).length ? `${fieldId}-${key}-note` : undefined} className={inputClass} type="number" min="0" placeholder="Unknown" value={values[key]?.[year] ?? ''} onChange={e=>onChange(key)({...values[key],[year]:e.target.value===''?null:Number(e.target.value)})}/></label>)}</fieldset>;
  ...
  return <details data-review-owner={owner} data-settings-title={title} className="financial-details my-4 rounded border border-slate-200 bg-slate-50 p-3">
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      {!household && <>
        ...
        <details data-review-field="ssMonthlyEarnings" className="sm:col-span-2"><summary className="text-sm cursor-pointer">Monthly W-2 earnings for the Social Security test</summary>{marker('ssMonthlyEarnings')}
